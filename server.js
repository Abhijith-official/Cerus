...
//distributors_logged
router.get('/distributor_logged',
  require('connect-ensure-login').ensureLoggedIn({ redirectTo: '/app/auth/login' }),
  function (req, res) {
    res.render('distributor_logged', { user: req.user, page_name: 'distributor' });
  })
  ...
  function (req, res) {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        let harvest_sql = "SELECT harvest_logid, supplierproduce, harvest_quantity, harvest_supplierName, harvest_produceName, harvest_unitofmeasure, harvest_TimeStamp  FROM foodprint_harvest WHERE harvest_logid='" + req.body.search_term +
